...
import { Accordion, useAccordionButton, AccordionContext, ListGroup, ProgressBar } from "react-bootstrap";
              ...
              <ListGroup.Item key={index} as="li" className="p-0">
                ...
                <Accordion.Collapse eventKey={item.id}>
                  <ListGroup variant="flush">
                    <ListGroup.Item className="fs-5  rounded-3">{item.summary}</ListGroup.Item>
                  </ListGroup>
                </Accordion.Collapse>
              </ListGroup.Item>
            );
                  ...
                  <ListGroup variant="flush">
                    ...
                    {item.topics.map((subitem, index) => (
                      <ListGroup.Item key={index} active={subitem.status === "continue"} disabled={subitem.locked}>
                        <Link to="#" className={`d-flex justify-content-between align-items-center text-${subitem.status === "continue" ? "white" : "inherit"} text-decoration-none`}>
                          <div className="text-truncate ">
                            <span
                              className={`icon-shape bg-${subitem.status === "continue" || subitem.locked ? "light" : subitem.status === "finished" ? "success" : "light"} text-${
                                subitem.locked ? "muted" : subitem.status === "continue" || subitem.status === "pending" ? "primary" : "white"
                              } icon-sm rounded-circle me-2`}
                            >
                              {subitem.locked ? <i className="fe fe-lock fs-4"></i> : <Icon path={mdiPlay} size={0.8} />}{" "}
                            </span>

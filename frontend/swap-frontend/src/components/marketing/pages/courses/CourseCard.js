...
import { Image, Card, Row, Col, ProgressBar, ListGroup, Badge } from "react-bootstrap";
...
const CourseCard = ({ item, free, viewby, showprogressbar, extraclass }) => {
      ...
      <Card className={`mb-4 card-hover ${extraclass}`}>
        ...
        <Card.Footer>
          ...
          <span className={`${showprogressbar ? "" : "d-none"}`}>
            {" "}
            <ProgressBar variant="success" now={item.progress} className="mt-3" style={{ height: "5px" }} />
          </span>
        </Card.Footer>
      </Card>
    );
      ...
      <Card className="mb-4 card-hover">
        <Row className="g-0">
          <Link
            ...
            lg={3}
            xs={12}
            className="bg-cover img-left-rounded"
            style={{
            ...
            to="#"
          >
            <Image src={item.image} alt="..." className="img-fluid d-lg-none invisible" />
          </Link>
    ...
    return (
      <div className="d-lg-flex align-items-center">
        ...
        <div className="ms-lg-3 mt-2 mt-lg-0">
          ...
          <ListGroup as="ul" bsPrefix="list-inline" className="fs-6 mb-0 text-inherit">
            <ListGroup.Item as="li" bsPrefix="list-inline-item">
              <i className="far fa-clock me-1"></i>
              {item.duration}
            </ListGroup.Item>
  ...
  return <Fragment>{viewby === "grid" ? <GridView /> : viewby === "list" ? <ListView /> : <ListGroupView />}</Fragment>;
};
